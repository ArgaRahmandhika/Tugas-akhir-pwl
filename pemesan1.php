...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $path_selfie = "image/penerima/" . basename($foto_selfie);

    // Upload ID Card
    $id_card = $_FILES['id_card']['name'];
    $tmp_idcard = $_FILES['id_card']['tmp_name'];
    $path_idcard = "image/penerima/" . basename($id_card);
    ...
    if (move_uploaded_file($tmp_selfie, $path_selfie) && move_uploaded_file($tmp_idcard, $path_idcard)) {
       $sql = "INSERT INTO penerima1
(gender, Nama_Penerima, No_KTP_VISA_PASSPORT, NO_TLP, Negara, Kota, Alamat, code, Selfie, Upload_KTP_VISA_PASSPORT)
VALUES 
('$gender', '$nama', '$no_identitas', '$no_tlp', '$negara', '$kota', '$alamat', '$code', '$foto_selfie', '$id_card')";
        if (mysqli_query($koneksi, $sql)) {
            echo "<script>alert('Data berhasil disimpan'); window.location.href='barang.php';</script>";
        } else {
            echo "Gagal menyimpan ke database: " . mysqli_error($koneksi);
        }
    } else {
        echo "Gagal upload file. Pastikan folder 'image/penerima/' memiliki izin tulis (chmod 755 atau 777).";
    }
}
?>
...
<head>
    ...
    <title>Form Penerima - INDO CARGO EXPORTS</title>
    <style>
        body {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="form-container">
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-title">PENERIMA</div>
            ...
            <div class="form-buttons">
                <button class="btn-back" type="button" onclick="window.location.href='pemesan.php';">KEMBALI</button>
